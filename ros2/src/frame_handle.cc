...
frame_t *handle_process(std::string s) {
  std::vector<uint8_t> &vecChar = frame_buffer();
  static constexpr size_t kMaxBufferSize = 3 * 12000;
  ...
  if (vecChar.size() > kMaxBufferSize) {
    vecChar.clear();
    return NULL;
  }
...
void reset_frame_parser() { frame_buffer().clear(); }
